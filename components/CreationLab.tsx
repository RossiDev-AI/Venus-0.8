...
// Import CinemaVaultModal to support vault selection in Creation Hub
...
const CreationLab: React.FC<CreationLabProps> = ({ onResult, params, setParams, onReset, vault = [], settings }) => {
  ...
  // FIX: Added injectedImage state to track base DNA for synthesis as required by CreationAction
  const [injectedImage, setInjectedImage] = useState<string | null>(null);
  // Track vault picker state for DNA injection
  ...
  const handleHardReset = () => {
    ...
    setLogs([]);
    setDeliberationFlow([]);
    // FIX: Reset injected image on hard reset
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // FIX: Pass injectedImage as baseImage to executeGroundedSynth to maintain visual continuity if present
  ...
  return (
    <div className="h-full flex flex-col bg-[#050505] overflow-hidden min-h-full">
        ...
        <div className="flex-1 space-y-10 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             ...
             <div className="space-y-8 flex flex-col">
                ...
                />
                {/* FIX: Passed injectedImage, onImageUpload, and onOpenVault props to resolve property missing errors */}
                <CreationAction 
      ...
      {/* Added Vault Picker Modal for Creation Hub */}
      {showVaultPicker && (
        <CinemaVaultModal 
          items={vault} 
