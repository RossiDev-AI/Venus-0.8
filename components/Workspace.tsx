...
// Added missing optimizeVisualPrompt import
...
// Import CinemaVaultModal to allow selecting nodes from vault
import CinemaVaultModal from './CinemaVaultModal';
...
const Workspace: React.FC<WorkspaceProps> = ({ 
  ...
  // Added isOptimizing state to fix missing prop error in StudioActionPanel
  ...
  useEffect(() => {
    let interval: any;
    // Added isOptimizing to loading check for UI consistency
    if (isProcessing || isBiopsyActive || isOptimizing) {
      setLoadingStep(0);
  ...
  }, [isProcessing, isBiopsyActive, isOptimizing]);

  // Shared biopsy execution logic for both upload and vault selection
  const runBiopsyOnImage = async (imgUrl: string) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = async (ev) => {
        ...
        runBiopsyOnImage(result);
      };
      reader.readAsDataURL(file);
    }
  };

  // Handle selection of existing vault item for studio injection
  const handleVaultSelect = (item: VaultItem) => {
    setCurrentImage(item.imageUrl);
    setOriginalSource(item.imageUrl);
    setShowVaultPicker(false);
    runBiopsyOnImage(item.imageUrl);
  };
  ...
  // Added handleOptimizePrompt to fix missing onOptimize prop in StudioActionPanel
  const handleOptimizePrompt = async () => {
    if (!prompt.trim() || isOptimizing) return;
    setIsOptimizing(true);
    setLogs([{ type: 'Meta-Prompt Translator', status: 'processing', message: 'Generating Industrial Manifesto based on current DNA...', timestamp: Date.now(), department: 'Advanced' }]);
    
    try {
      const optimized = await optimizeVisualPrompt(prompt, settings, currentImage || undefined);
      setPrompt(optimized);
      setLogs(prev => [...prev, { type: 'Meta-Prompt Translator', status: 'completed', message: 'Industrial Prompt Locked. Ready for Synth.', timestamp: Date.now(), department: 'Advanced' }]);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="h-full flex flex-col bg-[#0c0c0e] relative overflow-hidden min-h-full">
      ...
      <div className="flex-1 flex flex-col md:flex-row relative">
        {/* Added onOpenVault to StudioPreview to satisfy required prop */}
        <StudioPreview 
        ...
        <div className="w-full md:w-[380px] lg:w-[440px] flex flex-col bg-[#0e0e11] border-l border-white/5 shadow-2xl relative overflow-hidden">
          ...
          {/* Fix: Added targetDomain, setTargetDomain, onOptimize and isOptimizing props as required by StudioActionPanel */}
          <StudioActionPanel 
            prompt={prompt} 
            setPrompt={setPrompt} 
      ...
      {/* Render CinemaVaultModal for vault item selection */}
      {showVaultPicker && (
        <CinemaVaultModal 
          items={vault} 
